...
with DAG('BackendBI_Auditoria_BillingInfo', schedule_interval='0 6 * * *', catchup=False, default_args=default_args) as dag:
    # Llamado al auditor de billing info en datanode 2
    runBillingInfoAuditor = SSHOperator(
        task_id="BillingInfoAuditor",
        command="""
        /usr/bin/bash /home/hduser/backendbi-audit/BillingInfo/BillingInfoSyncValidation/BillingInfoSyncValidation_run.sh
        """,
        timeout = 20,
        ssh_conn_id = "ssh_hadoop_datanode2_ti"
    )

    runBillingInfoAuditor
