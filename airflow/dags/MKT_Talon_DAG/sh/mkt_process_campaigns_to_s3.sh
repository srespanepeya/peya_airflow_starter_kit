...
if [ -z "$1" ]
then
        echo "--->Begin ETL Load Campaigns into S3"
        ...
        echo "<---End ETL Load Campaigns into S3"
else
        echo "--->Begin ETL Load Campaigns into S3-$1"
        PYSPARK_PYTHON=/usr/bin/python3 /home/hduser/spark/bin/spark-submit --master spark://hadoop-namenode-ti:7077 --driver-memory 8G --driver-cores 4 --executor-memory 6G --conf spark.cores.max=4 /home/hduser/spark/apps/mkt_process_campaigns_to_s3.py -f $1
        echo "<---End ETL Load Campaigns into S3-$1"
fi
